(() => {
  ...
  const platformNames = { windows: 'Windows', macos: 'macOS', android: 'Android', linux: 'Linux', other: 'Other' };
  ...
  fetch(`${API_URL}?t=${Date.now()}`, { cache: 'no-store', credentials: 'same-origin' })
    ...
    .then(data => {
      ...
      document.title = `${product.title} — Kianan Bibika`;

      const gallery = (product.gallery || []).map(src => `
        <figure class="media-frame product-gallery-item"><img src="${escapeHtml(assetUrl(src))}" alt="${escapeHtml(product.title)} screenshot" loading="lazy"></figure>
      `).join('');
      ...
      if (distributionType === 'github-releases') {
        ...
        if (releasesUrl) primary += `<a class="button button-secondary" data-bibika-public="true" href="${escapeHtml(releasesUrl)}" target="_blank" rel="noopener">GitHub Releases</a>`;
      } else if (distributionType !== 'none' && product.links?.primaryUrl) {
        primary = `<a class="button button-primary" data-bibika-public="true" href="${escapeHtml(product.links.primaryUrl)}" target="_blank" rel="noopener">${escapeHtml(product.links.primaryLabel || 'Open link')}</a>`;
      }

      const downloads = validDownloads.map(({ item, asset }) => {
        const platform = platformNames[item.platform] || item.platform || 'Download';
        const details = [item.version ? `Version ${item.version}` : '', architectureName(item.platform, item.architecture), item.requirements || item.minOs || '', asset.extension || '']
          .filter(Boolean).map(escapeHtml).join(' · ');
        return `<article class="download-card"><div class="download-card-copy"><span class="download-platform">${escapeHtml(platform)}</span><h3>${escapeHtml(item.label || defaultDownloadLabel(item.platform))}</h3>${details ? `<p>${details}</p>` : ''}</div><a class="button button-primary download-button" data-bibika-public="true" href="${escapeHtml(asset.url)}" rel="noopener">Download</a></article>`;
      }).join('');

      shell.innerHTML = `
        <a class="back-link" href="category.html?category=${encodeURIComponent(product.category || '')}">← Back to ${escapeHtml(category?.title || 'catalog')}</a>
        <section class="product-detail-hero">
          <div class="product-detail-copy">
            <span class="eyebrow">${escapeHtml(category?.title || '')}</span>
            <h1>${escapeHtml(product.title)}</h1>
            <div class="product-labels">${product.status ? `<span>${escapeHtml(product.status)}</span>` : ''}${product.version ? `<span>Version ${escapeHtml(product.version)}</span>` : ''}</div>
            <p class="product-lead">${escapeHtml(product.description || product.shortDescription || '')}</p>
            <div class="hero-actions">${primary}</div>
          </div>
          <div class="product-cover-panel">${product.cover ? `<img src="${escapeHtml(assetUrl(product.cover))}" alt="${escapeHtml(product.title)}">` : `<div class="item-placeholder large">${escapeHtml(product.title.slice(0,2).toUpperCase())}</div>`}</div>
        </section>
